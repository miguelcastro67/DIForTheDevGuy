...
namespace Lib
{
    public class SuperheroService2 : ISuperheroService
    {
    ...
    public class DecoratedSuperheroService : ISuperheroService
    {
        public DecoratedSuperheroService(IAvengerRepository avengerRepository, ILogger logger)
        {
            _Decorated = new SuperheroService2(avengerRepository, logger, "miguel");
        }

        private readonly ISuperheroService _Decorated;
        
        public Hero GetAvenger(string name)
        {
            return _Decorated.GetAvenger(name);
        }

        public IEnumerable<Hero> GetAvengers()
        {
            return _Decorated.GetAvengers();
        }
    }
}
